...
export default function TeamDetail() {
  ...
  if (loading) return <div className="loading-container">Loading team...</div>;
  if (error) return <div className="error-container">Error: {error}</div>;
  if (!team) return <div className="error-container">Team not found</div>;
  ...
  return (
    <div className="team-detail-page">
      ...
      <div className="team-detail-header">
        <TeamHeader name = {team.event.name} date = {'TBA'} venue = {'TBA'} teamSize={team.event.min_team_size+'-'+team.event.max_team_size} description={team.event.intro} backgroundImage={img1}/>
      </div>
        ...
        <div className="team-card">
          ...
          <div className="member-list">
            {activeMembers.map((member) => (
              <div 
                key={member.id} 
                className={`member-item ${member.status === "PENDING" ? "member-pending" : ""}`}
              >
                ...
                <div className="member-right">
                  {member.is_leader && <span className="badge-leader">Leader</span>}
                  {isLeader && !member.is_leader && member.status !== "LEFT" && (
                    <button 
          ...
          {/* Accept Button for Pending Members */}
          ...
          {/* Leave Button for Non-Leader Accepted Members */}
          {currentMember && !currentMember.is_leader && currentMember.status === "ACCEPTED" && (
            <button className="btn-leave-team" onClick={handleLeave}>
              Leave Team
            </button>
          )}
        </div>
